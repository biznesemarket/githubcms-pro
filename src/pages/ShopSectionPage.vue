...
const sectionProducts = computed(() =>
  (pages as any)
    .filter((p: any) => p.frontmatter?.layout === "product" && p.frontmatter?.section === props.section)
    .map((p: any) => ({
      name: p.frontmatter?.title || "",
      desc: p.frontmatter?.description || "",
      price: p.frontmatter?.price || 0,
...
const sectionData = computed(() => {
  const numMap: Record<string, number> = { "shop-section-1": 1, "shop-section-2": 2, "shop-section-3": 3, "shop-section-4": 4, "shop-section-5": 5 };
  const n = numMap[props.section] || 1;
  return (t.shop as any).sectionDetails?.[n] as { title: string; heroSubtitle: string; description: string; icon: string; categories: string[] } | undefined;
});

const sectionTitle = computed(() => sectionData.value?.title || props.section);
...
useSeo(() => {
  const path = `/shop/${props.section}/`;
  return { title: `${sectionTitle.value} | ${siteConfig.name}`, description: `Shop section: ${sectionTitle.value}`, path, type: "website" };
});
</script>
